...
```{r, include=FALSE}
kobe_img <- load.image("data/kobe_bw.jpg")
```
...
Es mucho más intuitivo trabajar con _dataframes_, por lo que aprovecharemos para convertir el objeto `cimg` a `data.frame`. Por defecto el método de la función `as.data.frame()` convierte las imágenes a un data frame largo (por la manera en la que se R representa las imágenes), de modo que usaremos  la función `tidyr::spread()` para ponerla en formato ancho (aun no sé hacer PCA con data en formato tidy y tampoco me he dedicado a usar la función `tidyr::pivot_wider()` que sustituyó a `spread()`).   
...
En esta parte nos auxiliaremos del [blogpost de Kieran Healy](https://kieranhealy.org/blog/archives/2019/10/27/reconstructing-images-using-pca/) que en su momento realizó este ejercicio para una imagen de Elvis y el presidente Nixon.
...
Para una explicación bien detallada y estructurada de esta técnica, les recomiendo el capítulo 6.3 del libro [An Introduction to Statistical Learning](http://faculty.marshall.usc.edu/gareth-james/ISL/ISLR%20Seventh%20Printing.pdf).
